...
import type { CandyClassEditor, CandyInt8 } from '../../../types';
import { convertToInt8 } from './converters';
import { VALIDATION_ERRORS } from '../../../constants';

export const Int8Form = (editor: CandyClassEditor) => {
  const [name, setName] = useState<string>('');
  const [value, setValue] = useState<CandyInt8>();
  ...
  return (
    <Container>
      ...
      {isInvalid && (
        <>
          <Flex>{VALIDATION_ERRORS.INT8}</Flex>
          <HR marginTop={8} marginBottom={16} />
        </>
      )}
      <Flex>
