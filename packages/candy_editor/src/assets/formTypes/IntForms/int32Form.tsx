...
import type { CandyClassEditor, CandyInt32 } from '../../../types';
import { convertToInt32 } from './converters';
import { VALIDATION_ERRORS } from '../../../constants';

export const Int32Form = (editor: CandyClassEditor) => {
  const [name, setName] = useState<string>('');
  const [value, setValue] = useState<CandyInt32>();
  ...
  return (
    <Container>
      ...
      {isInvalid && (
        <>
          <Flex>{VALIDATION_ERRORS.INT32}</Flex>
          <HR marginTop={8} marginBottom={16} />
        </>
      )}
      <Flex>
