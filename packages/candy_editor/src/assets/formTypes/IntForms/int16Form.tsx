...
import type { CandyClassEditor, CandyInt16 } from '../../../types';
import { convertToInt16 } from './converters';
import { VALIDATION_ERRORS } from '../../../constants';

export const Int16Form = (editor: CandyClassEditor) => {
  const [name, setName] = useState<string>('');
  const [value, setValue] = useState<CandyInt16>();
  ...
  return (
    <Container>
      ...
      {isInvalid && (
        <>
          <Flex>{VALIDATION_ERRORS.INT16}</Flex>
          <HR marginTop={8} marginBottom={16} />
        </>
      )}
      <Flex>
