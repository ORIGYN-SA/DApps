...
import type { CandyClassEditor, CandyNat } from '../../../types';
import { convertToNat } from './converters';
import { VALIDATION_ERRORS } from '../../../constants';

export const NatForm = (methods: CandyClassEditor) => {
  const [name, setName] = useState<string>('');
  const [value, setValue] = useState<CandyNat>();
  ...
  return (
    <Container>
      ...
      {isInvalid && (
        <>
          <Flex>{VALIDATION_ERRORS.NAT}</Flex>
          <HR marginTop={8} marginBottom={16} />
        </>
      )}
      <Flex>
