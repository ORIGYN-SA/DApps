...
test('Fire button',async () => {
  const { getByTestId } = render(<Wrapper/>);
  await expect(
      async () => await fireEvent.click(getByTestId('switch-canister-button'))
  ).toBeTruthy();
...
test("No log if nothing is fired", () => {
...
test("Call log with correct message", async () => {
  const { getByTestId } = render(<Wrapper/>);
  fireEvent.click(getByTestId('switch-canister-button'));
  await waitFor(() => expect(console.log).toHaveBeenCalledWith("Not a valid canister"));
}
);
