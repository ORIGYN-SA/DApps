...
const NFTPage = () => {
  const { tokenId, canisterId, actor } =
  ...
  useEffect(() => {
    ...
    actor?.nft_origyn(tokenId).then((data) => {
      ...
      const publicApp =
        apps.value.Array.thawed
          .find((item) => item.Class
            .find(({name}) => name === "app_id").value.Text === "public.metadata");
      const publicData = publicApp.Class.find((data) => data.name === "data");

      const d = publicData.value.Class.reduce((data, current) => {
        return {...data, [current.name]: current.value.hasOwnProperty("Text") ? current.value.Text : current.value}
      }, {})
      const attachments = meta?.find((data) => data.name === "library").value.Array.thawed
        .filter((item) => item.Class
          .find(({name}) => name === "library_id").value.Text.includes("document"));

      const mintedMedia = meta?.find((data) => data.name === "library").value.Array.thawed
        .filter((item) => item.Class
          .find(({name}) => name === "library_id").value.Text.includes("minted"));
                  ...
                  description && (
                    <>
                      ...
                      <AccordionSection>
                        ...
                        <Grid container>
                          <Grid item xs={3}>Principal ID</Grid>
                          <Grid item xs={9}>
                            <b>{owner}</b>
                          </Grid>
                        </Grid>
                        <Grid container>
                          ...
                          <Grid item xs={9}><b>AlinCX</b></Grid>
                        </Grid>
                      </AccordionSection>
                    </>
                  )
            ...
            <Grid container>
              {
                images.map((img) => {
                  return (
                    <Grid item xs={12} md={4} key={img?.Class.find(({name}) => name === "location").value.Text}>
                      <img style={{maxWidth: "100%"}} src={img?.Class.find(({name}) => name === "location").value.Text} alt='' />
                    </Grid>
                  )
                })
              }
              ...
              <>
                <AccordionSection>
                  ...
                  <Grid container>
                    ...
                    <Grid item xs={9}><b>{metadata?.Brand} {metadata?.Model}</b></Grid>
                  </Grid>
                  <Grid container>
                    <Grid item xs={3}>Year</Grid>
                    <Grid item xs={9}><b>{metadata?.Year}</b></Grid>
                  </Grid>
                  {/*<Grid container>*/}
                ...
                <AccordionSection>
                  <h3>Case</h3>
                  <Grid container>
                    <Grid item xs={3}>Materials</Grid>
                    <Grid item xs={9}><b>{metadata?.Materials}</b></Grid>
                  </Grid>
                  <Grid container>
                    <Grid item xs={3}>Glass</Grid>
                    <Grid item xs={9}><b>{metadata?.Glass}</b></Grid>
                  </Grid>
                  <Grid container>
                    <Grid item xs={3}>Back</Grid>
                    <Grid item xs={9}><b>{metadata?.Back}</b></Grid>
                  </Grid>
                  <Grid container>
                    <Grid item xs={3}>Shape</Grid>
                    <Grid item xs={9}><b>{metadata?.Shape}</b></Grid>
                  </Grid>
                  <Grid container>
                    <Grid item xs={3}>Diameter</Grid>
                    <Grid item xs={9}><b>{metadata?.Diameter}</b></Grid>
                  </Grid>
                  <Grid container>
                    <Grid item xs={3}>Height</Grid>
                    <Grid item xs={9}><b>{metadata?.Height}</b></Grid>
                  </Grid>
                  <Grid container>
                    <Grid item xs={3}>W/R</Grid>
                    <Grid item xs={9}><b>{metadata["Water Resistance"]}</b></Grid>
                  </Grid>
                </AccordionSection>
                <hr/>
                <AccordionSection>
                  <h3>Dial</h3>
                  <Grid container>
                    <Grid item xs={3}>Nickname</Grid>
                    <Grid item xs={9}><b>{metadata?.Nickname}</b></Grid>
                  </Grid>
                  <Grid container>
                    <Grid item xs={3}>Color</Grid>
                    <Grid item xs={9}><b>{metadata?.Color}</b></Grid>
                  </Grid>
                  <Grid container>
                    <Grid item xs={3}>Finish</Grid>
                    <Grid item xs={9}><b>{metadata?.Finish}</b></Grid>
                  </Grid>
                  <Grid container>
                    <Grid item xs={3}>Indexes</Grid>
                    <Grid item xs={9}><b>{metadata["Index Type"]}</b></Grid>
                  </Grid>
                  <Grid container>
                    <Grid item xs={3}>Hands</Grid>
                    <Grid item xs={9}><b>{metadata?.Hands}</b></Grid>
                  </Grid>
                </AccordionSection>
                <br/>
              </>
          ...
          </Accordion>
          {
            history && (
              <AccordionSection>
                <History>
                  <h3>01/02/2022</h3>
                  <p><b>Biometry Capture</b></p>
                  <p>Organisation name</p>
                  <hr/>
                  <h3>22/12/2021</h3>
                  <p><b>Watch Service</b></p>
                  <p>Organisation name</p>
                  <hr/>
                  <h3>22/10/2020</h3>
                  <p><b>NFT Creation</b></p>
                  <p>Organisation name</p>
                </History>
              </AccordionSection>
            )
                    ...
                    <>
                      <FlexBlock>
                        <FlexBlock style={{gap: 14}}>
                          <DocumentIcon/>
                          <div>
                            <p><b><span style={{color: "#363636"}}>
                              {doc.Class.find(({name}) => name === "filename").value.Text.split(".")[0]}
                            </span>.{doc.Class.find(({name}) => name === "filename").value.Text.split(".")[1]}</b></p>
                            <p style={{fontSize: 14}}>
                              {(new Date(doc.Class.find(({name}) => name === "created_at")?.value?.Text))?.toUTCString()}
                            </p>
                          </div>
                        </FlexBlock>
