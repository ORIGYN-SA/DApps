...
const WalletPage = () => {
  ...
  const { loggedIn, tokenId, canisterId, principal, actor } =
  ...
  const createTableData = (data) => {
    ...
    const rows = data?.map((item) => {
      ...
      item?.metadata?.Class?.map((meta) => {
        if (!meta.name.startsWith('__'))
          if (meta?.value?.Text || meta?.value?.Principal) {
            rows[meta.name] =
              meta?.value?.Text ?? meta?.value?.Principal.toText()
          }
      })

      const sale_type =
        item?.current_sale.length > 0 ? item?.current_sale[0].sale_type : {}
      rows['sale'] =
        (
      ...
      rows['saleStatus'] =
        Object.keys(sale_type?.auction?.status || {})[0] || 'No sales'

      const isAuctionStarted =
        !sale_type?.auction?.status?.hasOwnProperty('closed') &&
        sale_type?.auction
      // @ts-ignore
  ...
  const fetchData = () => {
    if (actor && principal) {
      setIsLoading(true)
      actor
        ?.balance_of_nft_origyn({ principal: principal })
        .then((response) => {
          ...
          Promise.all(
            response?.ok?.nfts?.map((nft) => {
              return actor?.nft_origyn(nft).then((r) => r.ok)
            })
          )
            .then((data: any) => {
              const rows = []
              for (const item of data) {
                for (const sale of item.current_sale) {
                  console.log(sale)
                  const { start_price, buy_now, min_increase, token, ending } =
                    sale?.sale_type?.auction?.config?.auction || {}
                  const { status, current_bid_amount } =
                ...
                control={
                  <Checkbox
                    ...
                    onChange={(e: React.ChangeEvent<HTMLInputElement>) =>
                      setShowOnlyTokenEntries(e.target.checked)
                    }
                  />
                }
                label={
