...
const WalletPage = () => {
  ...
  const fetchData = async () => {
    ...
    setNormalData(data);
    if (data.status === STATUSES.assignedQR) {
      const responseNormalData = await fetch(
      ...
      console.log(STATUSES.assignedQR, code, loggedIn, "!@#");
      setClaimCode(code);
    }
    if ('prestage' in data) {
  ...
  return (
    <>
                          ...
                          normalData.status === STATUSES.assignedQR && (
                            <Grid columns={1} gap={16}>
                              <QRCode
                                ...
                                value={`https://development.canister.origyn.ch/canister/v0/token/${claimCode}`}
                                viewBox={`0 0 256 256`}
                              />
      ...
      <ConnectQRModal isOpen={isOpen} onClose={() => setIsOpen(false)} loggedIn={loggedIn} />
