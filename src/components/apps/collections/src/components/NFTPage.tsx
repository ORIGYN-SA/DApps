...
import data from '../../../ledger/src/components/TransactionsTable/tests/data';
...
const Banner = ({ nft }: { nft: NFT | undefined }) => {
  return (
    <div className="bg-charcoal flex justify-center items-center w-full h-10 fixed">
      <div>
        <div className="flex flex-row items-center gap-2">
          <span className="text-white text-xs font-bold leading-none">Collection</span>
          <span className="text-white text-xs font-medium"> – {nft?.collectionName}</span>
        </div>
      </div>
    </div>
  );
...
const Header = ({ nft, canisterId }: { nft: NFT | undefined; canisterId: string }) => {
  return (
    <div className="flex flex-row mt-10 pb-8 px-8 items-center border-b border-mouse ml-[88px]">
...
const NFTPage: React.FC<NFTPageProps> = () => {
  ...
  useEffect(() => {
    const fetchData = async () => {
      if (NFTid) {
        setLoading(true);
        // const data = await fetchNFTDetail(NFTid);
        const data = {
          nft: {
            id: NFTid,
            name: 'NFT Name',
        ...
        <div className="flex flex-col items-center w-full">
          <div className="w-full">
            ...
            <div className="xl:mt-10 3xl:mt-[92px] flex flex-col">
              <div className="flex flex-row bg-white rounded-2xl mx-auto border border-[#e1e1e1] max-w-6xl">
                <div className="">
                  <img
