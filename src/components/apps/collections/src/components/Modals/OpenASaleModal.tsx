...
const OpenASaleModal: React.FC<OpenASaleModalProps> = ({ selectedNFT, onClose }) => {
  ...
  const filteredCurrencies = useMemo(() => {
    return currencies.filter(curr => curr.code !== currency.code && curr.isUsable)
  }, [currency])
  ...
  const convertedPrice = useMemo(() => {
    return salePrice
      ...
      : '0.00'
  }, [salePrice, currency, getUSDPrice])

  const handleCurrencySelect = useCallback((selectedCurrency: Currency) => {
  ...
  }, [currency, salePrice, selectedNFT, sellNFT, queryClient, getTokenData])
  ...
  const renderPriceInput = useMemo(
    ...
    [salePrice, currency, convertedPrice, filteredCurrencies, isDropdownOpen, handleCurrencySelect],
  )
...
export default React.memo(OpenASaleModal)
