...
import { GovernanceIcon, LibraryIcon, ShopIcon } from '@dapp/common-assets'
...
const NavBar: React.FC = () => {
  ...
  const navItems: NavItemProps[] = [
    { name: 'Collection', Icon: ShopIcon, path: `/collection/${canisterId}` },
  ...
  const isActiveLink = (linkTo: string) => {
    if (linkTo === '/') {
      return currentPath === '/' || currentPath.startsWith(`/collection/${canisterId}`)
    }
    return currentPath.startsWith(linkTo)
  }

  const isOnRootCollection = useMemo(() => {
    const collectionRootPattern = new RegExp(`^/collection/${canisterId}(/)?$`)
    return collectionRootPattern.test(currentPath)
  }, [currentPath, canisterId])

  const NavItem: React.FC<{ item: NavItemProps; isActive: boolean }> = ({ item, isActive }) => {
    return (
      <Link
        to={isOnRootCollection && item.path.includes(`/collection/${canisterId}`) ? '#' : item.path}
        className={`flex flex-col items-center gap-1 ${isOnRootCollection ? 'cursor-default' : ''}`}
        onClick={e => isOnRootCollection && e.preventDefault()}
      >
        <div
