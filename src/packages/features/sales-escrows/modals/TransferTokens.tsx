...
import { validateTokenAmount, toSmallerUnit, toLargerUnit, toBigNumber } from '@dapp/utils';
  ...
  const sendTrx = async (data) => {
    try {
      ...
      if (values.token === 'ICP') {
        ...
        {
          accountData = {
            owner: Principal.fromText(data.recipientAddress),
            subaccount: [],
          };
        }
      }
      ...
      {success ? (
        ...
        <Container as="form" onSubmit={handleSubmit} size="full" padding="48px">
          ...
          <Flex flexFlow="column" gap={8}>
            ...
            />
            <br />
            <span>{values.token === 'OGY' ? 'Recipient Principal' : 'Recipient AccountID'}</span>
            <TextInput
